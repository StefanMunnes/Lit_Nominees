...
prizes <- c("aspekte", "leipziger", "oesterreich_debuet", "oesterreich",
...
dbp_raw <- read_xlsx("../data/prizes_xlsx/deutscher_buchpreis.xlsx", skip = 3) %>%
  ...
  mutate(Name = str_replace(Name, "(.+)(,\\ )(.+)", "\\3 \\1"),
...
prizes_df <- prizes_df |>
  mutate(across(c(shortlist, academic, institute, winner),
                ~ ifelse(str_detect(.x, "[jJ]a"), TRUE, FALSE)),
         female    = ifelse(female == "w", TRUE, FALSE),
         prize     = as.factor(prize),
         shortlist = ifelse(prize %in% c("aspekte", "leipziger", "oesterreich_debuet",
                                         "schweiz", "text_sprache"), NA, shortlist),
         language  = case_when(str_detect(language, "(Deutsch/)|(/Deutsch)") ~ "german+",
  ...
  separate(title, into = c("title", "subti"), sep = "[.][^.]*\\s*",
  ...
  mutate(
    ...
    subti = case_when(title == "Flut und Boden" ~ "Roman einer Familie",
