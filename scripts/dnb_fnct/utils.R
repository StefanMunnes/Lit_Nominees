...
dnb_to_df_sm <- function(lst, clean) {

  # prepare data.frame
  nrec <- length(lst$records)

  df <- data.frame(matrix(nrow = nrec, ncol = 26))
  names(df) <- c("id", "link", "author", "author_id", "author_year",
                 "contrib", "contrib_id", "title", "subtitle",
                 "publisher", "year", "language", "langorig", "isbn",
                 "price", "pages", "format", "edition", "keyword", "toc",
                 "description", "cover", "ddc", "subject", "genre", "topic")
  ...
  for(r in 1:nrec) {
    ...
    rec <- lapply(rec, function(x) unlist(x, recursive = F))
    ...
    if(!is.null(rec[["100"]][["subfield.a"]])) {	# author
      ...
      if(!is.null(rec[["100"]][["subfield.4"]])) {
        aut <- paste0(aut, " (", rec[["100"]][["subfield.4"]], ")")
      }
      df$author[r] <- aut
    }
    if(!is.null(rec[["100"]][["subfield.0"]])) {	# author-id
      df$author_id[r] <- rec[["100"]][["subfield.0"]]
    }
    if(!is.null(rec[["100"]][["subfield.d"]])) {	# author-year
      df$author_year[r] <- rec[["100"]][["subfield.d"]]
    }
    if(length(which(names(rec) == "700")) > 0) {	# contributer
      for(ca in which(names(rec) == "700")) {
        if(!is.null(rec[[ca]][["subfield.a"]])) {
          ...
          # get code
          if(!is.null(rec[[ca]][["subfield.4"]])) {
            coaut <- paste0(coaut, " (", rec[[ca]][["subfield.4"]], ")")
          }

          # write first or add
          if(is.na(df$contrib[r])) df$contrib[r] <- coaut
          else df$contrib[r] <- paste(df$contrib[r], coaut, sep="; ")
        }
      }
    }
    if(length(which(names(rec)=="700")) > 0) {	# contributer_id
      for(ca in which(names(rec)=="700")) {
        if(!is.null(rec[[ca]][["subfield.0"]])) {

          coaut_id <- rec[[ca]][["subfield.0"]]

          if(is.na(df$contrib_id[r])) df$contrib_id[r] <- coaut_id
          else df$contrib_id[r] <- paste(df$contrib_id[r], coaut_id, sep="; ")
        }
        else {
          coaut_id <- "NOID"
    ...
    if(!is.null(rec[["264"]][["subfield.b"]])) {	# publisher
      ...
      if(!is.null(rec[["264"]][["subfield.a"]]))
      df$publisher[r] <- paste0(pub, ", ", rec[["264"]][["subfield.a"]])
    }
    ...
    if(!is.null(rec[["041"]][["subfield.a"]])) {	# language
      df$language[r] <- paste(unlist(rec$`041`[names(rec[["041"]]) == "subfield.a"]), collapse = ";")
      # rec[["041"]][["subfield.a"]]
    }
    if(!is.null(rec[["041"]][["subfield.h"]])) {	# original language
      df$langorig[r] <- rec[["041"]][["subfield.h"]]
    }
    ...
    if(!is.null(rec[["082"]][["subfield.a"]])) {	# DDC - "Sachgruppe"
      df$ddc[r] <- paste(unlist(rec$`082`[names(rec[["082"]]) == "subfield.a"]), collapse = ";")
    }
    if(!is.null(rec[["083"]][["subfield.a"]])) {
      df$subject[r] <- paste(unlist(rec$`083`[names(rec[["083"]]) == "subfield.a"]), collapse = ";")
    }
    if(!is.null(rec[["084"]][["subfield.a"]])) {
      df$subject[r] <- paste(unlist(rec$`084`[names(rec[["084"]]) == "subfield.a"]), collapse = ";")
    }
    if(!is.null(rec[["655"]][["subfield.a"]])) { # genre
      df$genre[r] <- paste(unlist(rec$`655`[names(rec[["655"]]) == "subfield.a"]), collapse = ";")
    }
    if(!is.null(rec[["650"]][["subfield.a"]])) { # topic
      df$topic[r] <- paste(unlist(rec$`650`[names(rec[["650"]]) == "subfield.a"]), collapse = ";")
    }
  }
