...
prizes_dnb_unique <- left_join(prizes_df, dnb_books_all, by = c("match_id")) |>
  ...
  mutate(same_match_id = n(), # info about number of book variants
...
prizes_authors_id <- prizes_dnb_unique |>
  ...
  bind_rows(data.frame(author_search = c("Koeck, Thomas", "Boettcher, Bas"),
...
dnb_books_prize <- full_join(dnb_books_all, prizes_authors_id, by = "author_search") |>
  ...
  mutate(author_id_n = n_distinct(author_id),
  ...
  mutate(author_id_n = n_distinct(author_id_all, na.rm = TRUE),
  ...
  mutate(titles_multi = n_distinct(title) > 1,
  ...
  rename_with(~paste0(.x, "_dnb"),
              c(authors, author_id, title, subtitle, publisher, year,
                isbn, keyword, genre, ddc, subject, title_variants, contribs)) |>

  select(author, ends_with("_dnb"), match_id,
...
# Hausbesuch & Online ressourcen

# Seventeen selected poems
# language variable

#
# test <- mutate(dnb_books_prize,
#                match_id_end = paste0(clean(name), "_", str_sub(clean(title), -10, -1))) |>
#   group_by(match_id_end) |>
#   filter(n() > 1) |>
#   select(match_id_end, link, title, author_search)
#
#
# test2 <- filter(dnb_books_prize, str_detect(contribs, "\\(trl\\)")) |>
#   select(link, title, author_search, contribs, publisher, language, langorig, ddc)
