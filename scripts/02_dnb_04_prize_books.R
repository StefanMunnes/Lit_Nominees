...
dnb_books_prize <- dnb_books_all |>
  ...
  mutate(
    keep = case_when(
      ...
      is.na(author_id_all) &
        (str_detect(ddc, "(^8.+)|B") | str_detect(subject, "(^8.+)|B")) ~
        TRUE,
      TRUE ~ FALSE
    ),
    keep = case_when(
      ...
      str_detect(
        publisher,
        "Mouton|Helsingfors|Göteborg|Athena$|Athi̲na|Saint-Nazaire|Ōsaka|Wrocław|México|Chur|Athi̲na|Tel Aviv|London|New York|Manchester|San Diego|Paris|Todmorden|Maastricht|Eumo Editorial,|Argentinien|Austin|Shoshihanjitsukan|Todmorden|Montmorency|Devesset|Cairo|Černivci|Athēna|Pantin|São Paulo|Porto|Ourense|San Antonio|İstanbul|Vianen|Washington|Malmö|Zagreb|Bratislava|Antwerpen|Hanoi|Međunarodni|Taipei|København|Amsterdam|Stockholm|Kyôto|Oslo|RedFish|Warszawa|Poznań|Kraków|Madrid|Barcelona|Beograd|Viby|Milano|Roma$|Utrecht|Brussum|Baarn|Helsingissä|Espoo|Ljubljana|Budapest|Lisboa|Istanbul|Minnesota|Lausanne|Alidades|Holderbank SO|Helsingissä|Tallinn|Praha|Westbroek|Torino|Zaprešić|Alinea|Ronsosha|Bei jing|Zlín|Shang hai"
      ) ~
        FALSE,
  ...
  mutate(
    title = case_when(
      str_detect(title, "Abstecher") & author_search == "Walser, Martin" ~
        "Der Abstecher",
      str_detect(title, "Alle 7 Wellen") ~ "Alle sieben Wellen",
      str_detect(title, "Am Felsfenster morgens") ~
      ...
      str_detect(title, "Berlin in 100 Kapiteln") ~
        "Berlin in 100 Kapiteln - von denen leider nur 13 fertig wurden",
      ...
      str_detect(title, "Bulle & Bär: Der Weg des Schwerts") ~
      ...
      str_detect(title, "Das Glück ist eine Bohne") ~
        "Das Glück ist eine Bohne",
      str_detect(title, "Das habe ich jetzt akustisch nicht verstanden") ~
        "Das habe ich jetzt akustisch nicht verstanden",
      str_detect(title, "Das Schweigen am") ~
      ...
      str_detect(title, "((Der fremde Freund)|(Drachenblut))") ~
        "Der fremde Freund|Drachenblut",
      str_detect(title, "Der Goalie bin") ~ "Der Goalie bin ig",
      str_detect(title, "Der Grundri(ß|ss) von Grillparzer[s]* Bühnenkunst") ~
        "Der Grundriss von Grillparzers Bühnenkunst",
      str_detect(title, "Der Kotti") ~
        "Der Kotti - die Versteigerung von No. 36",
      str_detect(title, "Dialog Christ - Marxist") ~ "Dialog Christ-Marxist",
      str_detect(title, "Die Gallist_l28sche Krankheit") ~
        "Die Gallistl'sche Krankheit",
      str_detect(title, "Die Inseln im Landmeer und neue Gedichte") ~
        "Die Inseln im Landmeer",
      str_detect(title, "Die Lehmann-Trilogie") ~ "Die Lehmann Trilogie",
      str_detect(title, "Die Liebe zu Zeit") ~
        "Die Liebe zur Zeit des Mahlstädter Kindes",
      str_detect(title, "Die Rosenbaum-Doktrin") ~ "Die Rosenbaum-Doktrin",
      str_detect(title, "Die Ritter der Tafelrunde und andere Stücke") ~
      ...
      str_detect(title, "Duden, Schreiben dicht am Leben") ~
      ...
      str_detect(title, "Ein Flugzeug über dem Haus") ~
        "Ein Flugzeug über dem Haus und andere Geschichten",
      str_detect(title, "Ein Haus auf dem Land/ Eine Wohnung in der Stadt") ~
        "Ein Haus auf dem Land",
      str_detect(title, "Ein Mann von 40 Jahren") ~
        "Ein Mann von vierzig Jahren",
      str_detect(title, "Einladung zum Lever Bourgeoise") ~
        "Einladung zum Lever Bourgeois",
      str_detect(title, "Ende des Vogelgesanges") ~ "Ende des Vogelgesangs",
      str_detect(title, "Erfahrungen beim Verfassen einer Sonntagsrede") ~
        "Erfahrungen beim Verfassen einer Sonntagsrede",
      str_detect(title, "Es Härz für Gaukler") ~ "Ein Herz für Gaukler",
      str_detect(title, "Fährmann, hol über!") ~
      ...
      str_detect(title, "Geschmeidig, brutal, snobistisch und sexy") ~
        "Geschmeidig, brutal und sexy",
      str_detect(title, "Grand Hotel") ~ "Grandhotel",
      str_detect(title, "Hänsel und Gretel") ~
        "Hänsel und Gretel und der Hund und der Bär",
      str_detect(
        title,
        "Helene Hegemann über Patti Smith - KiWi Musikbibliothek, Band 13"
      ) ~
      ...
      str_detect(title, "Kaspar Mauser") ~
        "Kaspar Mauser - die Feigheit vorm Freund",
      str_detect(title, "Kirillov") ~ "Kirillow",
      str_detect(title, "Kristof Magnusson über Pet Shop Boys") ~
        "Kristof Magnusson über Pet Shop Boys, queere Vorbilder und musikalischen Mainstream",
      str_detect(title, "Lasalle fragt Herrn Herbert nach Sonja") ~
        "Lassalle fragt Herrn Herbert nach Sonja",
      str_detect(title, "Leeres Viertel Rub' Al -Khali") ~
        "Leeres Viertel - Rubʿ al-Khali",
      str_detect(
        title,
      ...
      str_detect(
        title,
        "Mal hören, was noch kommt/Jetzt, wo alles zu spät is. Zwei Erzählungen. Trivialroman"
      ) ~
      ...
      str_detect(
        ...
        "Meienbergs Tod",
      str_detect(title, "Mozart - im Innern seiner Sprachen") ~
        "Mozart im Innern seiner Sprachen",
      str_detect(title, "Meine Zeit, mein Tier") ~
      ...
      str_detect(title, "Sprung in den Papierkorb") ~
      ...
      str_detect(title, "Umwertung aller Werte?") ~
        "Umwertung aller Werte? Deutsche Literatur im Urteil Nietzsches",
      str_detect(title, "Warum die Sache schief geht") ~
        "Warum die Sache schiefgeht",
      str_detect(title, "Wie das Schwein zu Tanze ging") ~
        "Wie das Schwein zum Tanze ging", # check
      str_detect(title, "Wie der Soldat das") ~
        "Wie der Soldat das Grammofon repariert",
      str_detect(title, "Wunde r") ~ "WUNDER",
      str_detect(title, "Zmittst im Gjätt uss") ~ "Mitten im Nirgendwo",
      str_detect(title, "Zweisamkeit der Einzelgänger") ~
  ...
  mutate(
    ...
    title_short = case_when(
      # split on colon
      str_detect(title, "[:].+") ~
        str_extract(title, ".*?:") |> str_remove("[:]"),
      # split on dot (if not in front of Nr.)
      str_detect(title, "(?!N[ro])\\s*[.].+") ~
        str_extract(title, ".*?(?!N[ro])\\s*[.]") |> str_remove("[.]"),
      # split on hyphen
      str_detect(title, " [-].+") ~
        str_extract(title, ".*?-") |> str_remove("[-]"),
      # split on or (oder)
      str_detect(title, " oder.+") ~
        str_extract(title, ".*?oder") |> str_remove("oder"),
      TRUE ~ title
    ),
